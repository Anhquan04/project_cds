...
# ====== TAB 3: Prophet (chọn cột ngày thủ công) ======
with tab_prophet:
    st.subheader("Dự báo nhu cầu tuyển dụng theo thời gian (Prophet)")

    # Gợi ý cột ngày theo từ khoá + các cột object
    candidates = [c for c in filtered.columns
                  if any(k in c.lower() for k in ["date","posted","time","created","ngay","publish"])]
    more = [c for c in filtered.select_dtypes(include=["object"]).columns if c not in candidates]
    candidates = list(dict.fromkeys(candidates + more))  # unique, giữ thứ tự

    date_col = st.selectbox(
        "Chọn cột thời gian để dự báo",
        options=candidates,
        index=0 if candidates else None,
        placeholder="-- chọn cột --"
    )
    horizon_days = st.slider("Số ngày dự báo", min_value=7, max_value=180, value=90, step=7)
    agg_unit = st.selectbox("Gộp theo", options=["D","W","M"], index=0, help="D: ngày • W: tuần • M: tháng")

    if not date_col:
        st.info("Hãy chọn một cột thời gian (ví dụ: posted_date).")
    else:
        try:
            # Parse thời gian + tạo chuỗi đếm số tin theo mốc thời gian
            ts = filtered[[date_col]].copy()
            ts["ds"] = pd.to_datetime(ts[date_col], errors="coerce")
            ts = ts.dropna(subset=["ds"])
            if ts.empty:
                st.warning("Cột được chọn không parse được datetime.")
            else:
                ts["y"] = 1
                # Gộp theo ngày/tuần/tháng để đủ điểm dữ liệu
                if agg_unit == "D":
                    ts = ts.groupby(ts["ds"].dt.date, as_index=False)["y"].sum()
                    ts["ds"] = pd.to_datetime(ts["ds"])
                elif agg_unit == "W":
                    ts["ds"] = ts["ds"].dt.to_period("W").dt.start_time
                    ts = ts.groupby("ds", as_index=False)["y"].sum()
                else:  # "M"
                    ts["ds"] = ts["ds"].dt.to_period("M").dt.start_time
                    ts = ts.groupby("ds", as_index=False)["y"].sum()

                if len(ts) < 10:
                    st.warning("Quá ít điểm dữ liệu sau khi gộp. Hãy nới lỏng bộ lọc hoặc chọn gộp theo tuần/tháng.")
                else:
                    from prophet import Prophet
                    m = Prophet()
                    m.fit(ts)  # cần cột ds (datetime) và y (giá trị)
